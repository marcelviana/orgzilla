...
**How to apply:** ao auditar §3.1, `pessoas/nova`, `pessoas/[id]/editar`, `pessoas/[id]` detalhe, `perfil` (troca de senha), `configuracoes/cargos`, `configuracoes/trilhas` e `configuracoes/page.tsx` devem aparecer como ✅ ou ⚠️. `perfil` migrado via `atualizarSenhaAction` → `AuthService.atualizarSenha` (2026-06-01). `configuracoes/cargos` migrado via `getPessoasNoCargo` com `PessoaRepository.findByCargoId` + `PermissaoService.getTimesHierarquia` (2026-06-01). `configuracoes/trilhas` migrado via `getCargosNaTrilha`/`getPessoasNaTrilha` com filtro de hierarquia para gestor (2026-06-01). `configuracoes/page.tsx` migrado para Server Component com `getNiveisComEstatisticas`, `getTrilhasComEstatisticas`, `getTagsComEstatisticas`; histórico de login é mensagem estática "em breve" — marcado ⚠️ (2026-06-01). Padrão adotado: página landing virou Server Component + `configuracoes-client.tsx` como Client Component separado. Pendentes restantes de mock: `busca`, `relatorios`.
