...
**How to apply:** ao auditar §3.1, `pessoas/nova`, `pessoas/[id]/editar`, `pessoas/[id]` detalhe, `perfil` (troca de senha) e `configuracoes/cargos` devem aparecer como ✅. `perfil` migrado via `atualizarSenhaAction` → `AuthService.atualizarSenha` (2026-06-01). `configuracoes/cargos` migrado via `getPessoasNoCargo` (em `cargos.actions.ts`) que usa `PessoaRepository.findByCargoId` + `PermissaoService.getTimesHierarquia` para filtro de gestor (2026-06-01). Pendentes restantes de mock: `busca`, `relatorios`, `configuracoes/trilhas`, `configuracoes/page.tsx`.
